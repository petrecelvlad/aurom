...
*   `scripts/scrapeAndIngest.ts` runs `ProductAggregatorService.aggregateAll()` across the registered scrapers plus the BNR benchmark fetch, then `POST`s the result to `/api/ingest`. Avangard Gold is currently unregistered (paused pending a ToS permission reply — see `PROVIDER_SCRAPING_SPECS.md`), so 4 of 5 dealers run.
*   Triggered by `.github/workflows/scrape.yml` **once a day** (06:00 UTC), plus a manual `workflow_dispatch`. This is deliberately infrequent, not a technical ceiling — these dealers don't change prices more than once a day, and hitting their sites any more often than that isn't good scraping etiquette.
*   **Why not a Cloudflare Cron Trigger:** Workers Free plan caps a single invocation at 10ms of CPU time. That budget excludes I/O wait (the scrape requests themselves), but cheerio parsing ~15 dealer pages and `unpdf` extracting the BCR PDF are real CPU work that doesn't fit in 10ms regardless of how the ticks are split. Running the same scraper code under plain Node on a GitHub Actions runner has no such limit, and costs nothing on a public repo — this reasoning holds regardless of cadence, it's a separate concern from how often we choose to run it.
