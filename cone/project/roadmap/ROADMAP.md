...
- [x] **Cloudflare Migration**: Serving moved off Express to a Cloudflare Worker (D1-backed API + Workers Static Assets for the frontend) — the API only ever reads the last persisted snapshot, never scrapes on request. Scraping itself runs via GitHub Actions (`scripts/scrapeAndIngest.ts`, plain Node, ~every 5 minutes) POSTing into an authenticated `/api/ingest` route, not a Cloudflare Cron Trigger — Workers Free plan's 10ms CPU budget can't fit 5 scrapers + PDF parsing in one invocation, and GitHub Actions has no such limit and is free on this public repo. See [OVERVIEW.md](../architecture/OVERVIEW.md) and [HEXAGONAL.md](../architecture/HEXAGONAL.md).
