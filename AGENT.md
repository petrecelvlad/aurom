...
- **Read/write separation is load-bearing.** The Worker (`src/worker.ts`) never scrapes. Its `GET` routes only ever read from D1; its `POST /api/ingest` route only ever writes what it's handed, authenticated by a shared secret. Scraping happens in `scripts/scrapeAndIngest.ts`, run by GitHub Actions (`.github/workflows/scrape.yml`) on a schedule, entirely outside Cloudflare. This split exists because Workers Free plan's 10ms CPU budget can't fit 5 scrapers + PDF parsing in one invocation — it is also the actual mechanism that guarantees users never see an empty list, since the request path and the scraping path are fully decoupled. I don't add scraping calls to any code that runs on the Worker's request path, and I don't move scraping back into the Worker without reintroducing the CPU-budget problem this solved.
